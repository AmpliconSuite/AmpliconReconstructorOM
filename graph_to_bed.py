...
parser = argparse.ArgumentParser(
    description="Convert AA-formatted graph file to bed format of the segments in the graph")
parser.add_argument("-g", "--graph", help="AA-formatted graph file", required=True)
parser.add_argument("--padding", type=int,
                    help="padding to add on either side of graph regions, in bp. Used in creation of padded_sorted_merged file",
                    default=0)
parser.add_argument("--bedGraph", default=False, action='store_true',
                    help="Create a bedGraph (no padding) of the graph segment copy numbers")
